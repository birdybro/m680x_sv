"""Independent digital pin-phase model for the Hitachi HD63701V0 bus.

The mode roles and active transfers follow Hitachi #U07 sections 2.1, 2.4,
and 2.10 plus figures 3-11-1 and 3-11-2. Reset and low-power pin states
follow sections 2.8 and 2.12. Electrical and oscillator behavior is excluded.
"""

from __future__ import annotations

from dataclasses import dataclass


LEGAL_MODES = frozenset({0, 1, 2, 5, 6, 7})
MULTIPLEXED_MODES = frozenset({0, 2, 6})
FULL_UPPER_ADDRESS_MODES = frozenset({0, 1, 2})


@dataclass(frozen=True)
class HD63701V0BusInputs:
    mode: int
    phase: int
    phase_reset_n: bool
    reset_active: bool
    standby: bool
    address: int
    sleeping: bool = False
    waiting: bool = False
    write: bool = False
    port1: int = 0
    port1_oe: int = 0
    port2: int = 0
    port2_oe: int = 0
    port3: int = 0
    port3_oe: int = 0
    port4: int = 0
    port4_oe: int = 0
    os3_n: bool = True


@dataclass(frozen=True)
class HD63701V0BusPins:
    e: bool
    sc1: bool
    sc1_oe: bool
    sc2: bool
    port1: int
    port1_oe: int
    port2: int
    port2_oe: int
    port3: int
    port3_oe: int
    port4: int
    port4_oe: int


def bus_pins(inputs: HD63701V0BusInputs) -> HD63701V0BusPins:
    if inputs.mode not in LEGAL_MODES:
        raise ValueError("HD63701V0 operating mode must be 0, 1, 2, 5, 6, or 7")
    if inputs.phase not in range(4):
        raise ValueError("HD63701V0 bus phase must be in the range 0-3")

    multiplexed = inputs.mode in MULTIPLEXED_MODES
    mode1 = inputs.mode == 1
    mode5 = inputs.mode == 5
    single_chip = inputs.mode == 7
    idle = (inputs.sleeping or inputs.waiting) and not single_chip
    address = 0xFFFF if idle else inputs.address & 0xFFFF
    write = inputs.write and not (inputs.sleeping or inputs.waiting)

    e = inputs.phase_reset_n and not inputs.standby and inputs.phase >= 2
    sc1 = inputs.phase == 0 if multiplexed else True
    if mode5:
        sc1 = (address >> 8) != 0x01
    sc1_oe = multiplexed or mode5
    sc2 = (inputs.phase < 2 or inputs.os3_n) if single_chip else not write
    port1 = inputs.port1 & 0xFF
    port1_oe = inputs.port1_oe & 0xFF
    port2 = inputs.port2 & 0x1F
    port2_oe = inputs.port2_oe & 0x1F
    port3 = inputs.port3 & 0xFF
    port3_oe = inputs.port3_oe & 0xFF
    port4 = inputs.port4 & 0xFF
    port4_oe = inputs.port4_oe & 0xFF

    if not inputs.phase_reset_n:
        port1_oe = 0
        port2_oe = 0
        port3_oe = 0
        port4_oe = 0
        sc1_oe = False
        sc2 = True
    elif inputs.reset_active or inputs.standby:
        port1_oe = 0
        port2_oe = 0
        port3 = 0xFF
        port3_oe = 0
        port4_oe = 0
        sc1 = True
        sc2 = True
        if single_chip:
            sc1_oe = False
        elif mode5:
            sc1_oe = True
        else:
            sc1_oe = inputs.phase < 2
    elif single_chip:
        sc1_oe = False
    elif mode1:
        port1 = address & 0xFF
        port1_oe = 0xFF
        port4 = address >> 8
        port4_oe = 0xFF
        port3_oe = (
            inputs.port3_oe & 0xFF
            if inputs.phase >= 2 and not (inputs.sleeping or inputs.waiting)
            else 0
        )
        sc1_oe = False
    elif mode5:
        port4 = address & 0xFF
        port3_oe = (
            inputs.port3_oe & 0xFF
            if inputs.phase >= 2 and not (inputs.sleeping or inputs.waiting)
            else 0
        )
    elif multiplexed:
        port4 = address >> 8
        if inputs.mode in FULL_UPPER_ADDRESS_MODES:
            port4_oe = 0xFF
        if inputs.phase == 0:
            port3 = address & 0xFF
            port3_oe = 0xFF
        elif inputs.phase == 1:
            port3_oe = 0
        elif inputs.sleeping or inputs.waiting:
            port3_oe = 0

    return HD63701V0BusPins(
        e=e,
        sc1=sc1,
        sc1_oe=sc1_oe,
        sc2=sc2,
        port1=port1,
        port1_oe=port1_oe,
        port2=port2,
        port2_oe=port2_oe,
        port3=port3,
        port3_oe=port3_oe,
        port4=port4,
        port4_oe=port4_oe,
    )


@dataclass
class HD63701V0BusSequencer:
    phase: int = 0
    reset_active: bool = True

    def tick(
        self,
        *,
        clock_enable: bool = True,
        phase_reset_n: bool = True,
        reset_n: bool = True,
        standby_n: bool = True,
    ) -> int:
        if not phase_reset_n:
            self.phase = 0
            self.reset_active = True
            return self.phase
        if not reset_n:
            self.reset_active = True
        if not standby_n:
            self.phase = 0
            self.reset_active = True
            return self.phase
        if clock_enable:
            previous_phase = self.phase
            self.phase = (self.phase + 1) & 3
            if reset_n and previous_phase == 3:
                self.reset_active = False
        return self.phase

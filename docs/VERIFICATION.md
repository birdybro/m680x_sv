...
For 196 base-MC6800 encodings, the generated opcode regression compares every
...
immediate, direct, indexed, extended, and relative form. Checks include the
no-carry indexed address cycle, VMA-low read-modify-write and store phases,
TST's VMA-low R/W-low final cycle, every BSR stack/internal phase, and both JSR
sequences including the extended target prefetch and low-byte reread.
The same checks cover the register-update, push/pull, RTS, RTI, and SWI
sequences. WAI is excluded from exact bus-direction claims because Table 8's
ninth-cycle read conflicts with M68PRM/D section 3.2's required condition-code
stack write; architectural state, stack content, and the nine-cycle total are
still tested.
...
|---|---:|---:|
| M6800 | 6,689 | 225 |
| MC6800 bus wrapper | 6,760 | 231 |
| MC6800 four-subphase integration wrapper | 6,760 | 236 |
| MC6801 | 6,696 | 218 |

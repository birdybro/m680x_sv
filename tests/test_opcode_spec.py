...
class OpcodeSpecificationTests(unittest.TestCase):
    ...
    def test_m6800_table8_complete_bus_traces_are_structured(self) -> None:
        ...
        self.assertEqual(len(complete), 107)
        for record in complete:
            self.assertEqual(len(record["documented_bus_cycles"]), record["cycles"])
            self.assertEqual(
                [cycle["cycle"] for cycle in record["documented_bus_cycles"]],
        ...
        self.assertEqual(
            self.m6800["opcodes"][0x9C]["documented_bus_cycles"][2:],
            [
                {
                    "cycle": 3,
                    "address": "direct_effective_address",
                    "direction": "read",
                    "data": "operand_high",
                },
                {
                    "cycle": 4,
                    "address": "direct_effective_address+1",
                    "direction": "read",
                    "data": "operand_low",
                },
            ],
        ...
        self.assertEqual(
            self.m6800["opcodes"][0x7E]["documented_bus_cycles"][-1],
            {
                "cycle": 3,
                "address": "opcode_address+2",
                "direction": "read",
                "data": "extended_address_low",
            },
        )
